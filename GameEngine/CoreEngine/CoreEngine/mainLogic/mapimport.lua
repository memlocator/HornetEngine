...
local rayTracedTexture = GameObject.Textures.Create(resX, resY, Enum.SampleType.Nearest.Value, Enum.WrapType.ClampExtend.Value, Enum.DataType.Float.Value)
...
if true then
	...
	local MeshLoader = GameObject.MeshLoader
	local Transform = GameObject.Transform
	local Model = GameObject.Model
	local Light = GameObject.Light
			...
			if a > 1e-6 and current.shape ~= "Truss" and current.shape ~= "AlternatingTruss" and current.shape ~= "BridgeTruss" then
				if current.shape == "Special" then
					...
					if not loadedMeshes[current.fileid] then
						print(pcall(function()
						local mesh = MeshLoader.NewAsset(current.fileid, "./assets/maps/" .. mapName .. "/models/" .. current.fileid .. ".obj")
...
local viewframeDrawTexture = GameObject.Textures.Create(viewframeX, viewframeY, Enum.SampleType.Nearest.Value, Enum.WrapType.ClampExtend.Value, Enum.DataType.Float.Value)
local viewframeBuffer = GameObject.FrameBuffer.Create(viewframeX, viewframeY, viewframeDrawTexture, true)
...
viewframeBuffer.Parent = canvasParent
